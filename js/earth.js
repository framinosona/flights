  ...
  var promise = new Promise((resolve, reject) => {
    var texture = new BABYLON.Texture(
      ...
      () => {
        ...
        console.warn(
          `Failed to load texture for tile ${tileId.x},${tileId.y},${tileId.zoom}`
        );
        reject();
      }
    );
...
var addPoleCaps = function (scene) {
  // North pole cap
  ...
  northPoleCap.position = new BABYLON.Vector3(0, 1, 0); // adjust radius based on Earth size
  northPoleCap.rotation.x = Math.PI / 2;

  // South pole cap
  const southPoleCap = northPoleCap.clone("southCap");
    ...
    if (loadedTiles.length > 0 && !refinementActive) {
      ...
      if (Math.random() < 0.1) {
        ...
        if (mesh && mesh.tileId && !mesh.isDisposed()) {
          ...
          if (tileId.zoom < 5) {
            // Reduced max zoom to prevent excessive detail
            var children = tileId.children;
            ...
            Promise.all([child0, child1, child2, child3])
              .then(async () => {
                try {
